  ...
  return (
    <section className="py-24 bg-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid lg:grid-cols-[0.8fr_1.2fr] gap-0 border border-gold rounded-[0rem] overflow-hidden shadow-2xl bg-white">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="p-10 lg:p-16 bg-[#faf9f6]/50 border-r border-gold relative overflow-hidden"
          >
            ...
            <div className="space-y-4 relative z-10">
              {notPaidFor.map((item, index) => (
                <div
                  key={item.text}
                  className="flex items-center gap-4 p-10 bg-white/50 border border-sand opacity-40 grayscale group transition-all"
                >
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {paidForProof.map((item, index) => (
                <motion.div
                  ...
                  whileHover={{ y: -5 }}
                  className="flex items-center gap-4 p-6 bg-[#faf9f6] border border-sand hover:bg-white hover:border-gold-dark/30 hover:shadow-xl transition-all group"
                >
                  <div className="w-10 h-10 bg-white border border-sand rounded-xl flex items-center justify-center flex-shrink-0 group-hover:bg-gold-dark transition-colors duration-500">
                    <item.icon className="w-5 h-5 text-gold group-hover:text-white transition-colors" />
                  </div>

                  <span className="text-xs font-black text-warm-gray uppercase tracking-tight leading-snug">
                    {item.text}
                  </span>
                </motion.div>
              ))}
